...
% Define specific plot titles for Table 1 and Table 2
titlesTable1 = {"3 View L1 RH", "3 View L2 RH", "3 View L3 RH", "3 View L1 LH", "3 View L2 LH", "3 View L3 LH"};
titlesTable2 = {"1 View L1 RH", "1 View L2 RH", "1 View L3 RH", "1 View L1 LH", "1 View L2 LH", "1 View L3 LH"};

% Function to plot histogram and shaded distribution curve
function plot_hist_and_shaded_distribution(data, titleText)
    figure;
...
function plot_scatter_with_whiskers(averages, std_devs, labels, titleText)
    figure;
...
for col = 2:numel(titlesTable1) + 1 % Start from column 2
    col_data = table1(:, col);
    col_data = col_data(~isnan(col_data)); % Remove NaNs
    
    % Use predefined title
    plot_hist_and_shaded_distribution(col_data, titlesTable1{col - 1});
end

% Loop through each column and plot for Table 2 (excluding column I)
for col = 2:numel(titlesTable2) + 1 % Start from column 2
    col_data = table2(:, col);
    col_data = col_data(~isnan(col_data)); % Remove NaNs
    
    % Use predefined title
    plot_hist_and_shaded_distribution(col_data, titlesTable2{col - 1});
end

% Create scatter plot with whiskers for Table 1 averages
std_devs1 = std(table1, 0, 1); % Standard deviation for Table 1 (columns)
plot_scatter_with_whiskers(avgRow1, std_devs1, columnLabels1(2:end), '3 View Average');

% Create scatter plot with whiskers for Table 2 averages
std_devs2 = std(table2, 0, 1); % Standard deviation for Table 2 (columns)
plot_scatter_with_whiskers(avgRow2, std_devs2, columnLabels2(2:end), '1 View Average');
